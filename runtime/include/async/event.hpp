		...
		*/
		...
		/*!	\brief Блокирование текущего фибера до наступления сигнального состояния
			...
			\warning Поток (thread) может изменится во время исполнения этого метода.
				Следует быть осторожным при использовании компонент, которые рассчитывают на 
				неизменность потока, например, boost::mutex с его lock unlock. 
				Пример проблемы:
				\code
					boost::mutex mtx;
					async::Event myEvent;
...

					{
						...
						myEvent.wait();
					}//тут произойдет освобождение мутекса и если поток изменился - программа 
					//завершится аварийно
				\endcode
		*/
		void wait();
