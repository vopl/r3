...
#define RST_ENTRY(i, t, n) std::map<std::string, SF##n> use_##n##s;
#include "rawSimpleTypes/list.h"
...
#define RST_ENTRY(i, t, n) \
		case dt2_##n: \
		{ \
			typedef std::map<std::string, SF##n>::const_iterator iterator; \
			iterator const it = wrapper.use_##n##s.find(name); \
			name_exists = (it != wrapper.use_##n##s.end()); \
		} //\
...
#define RST_ENTRY(i,t,n) \
				case dt2_##n: \
					wrapper->st.exchange( \
						into(wrapper->into_##n##s[i], wrapper->into_indicators[i])); \
					break;
#include "rawSimpleTypes/list.h"
...
#define RST_ENTRY(i,t,n) \
				case dt2_##n: \
					wrapper->st.exchange( \
						into(wrapper->into_##n##s_v[i], wrapper->into_indicators_v[i])); \
					break;
#include "rawSimpleTypes/list.h"
...
#define RST_ENTRY(i,t,n) \
        { \
			typedef std::map<std::string, SF##n>::iterator iterator; \
			iterator uit = wrapper->use_##n##s.begin(); \
            iterator const uend = wrapper->use_##n##s.end(); \
            for ( ; uit != uend; ++uit) \
            { \
                std::string const & use_name = uit->first; \
				SF##n & use_val = uit->second; \
                indicator & use_ind = wrapper->use_indicators[use_name]; \
                wrapper->st.exchange(use(use_val, use_ind, use_name)); \
            } \
        }
#include "rawSimpleTypes/list.h"
